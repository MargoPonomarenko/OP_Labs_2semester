...
string createFile()   //створення основного файлу
{
	...
	string line;
	do {   //зчитування даних для їх запису у файл
		getline(cin, line);
		if (line != "") {
			file << line + "\n";
		}

	} while (line != "");   //зчитування до першого пустого рядка
	file.close();
	return fileName;
}
...
vector<string> split(const string& line, char splitter)   //виділення слів з рядку
{
	vector<string> temp;
	string tempLine = "";
	if (line.length() == 0) {
...
string toString(const vector<string>& data)   //приймання відсортованого вектору зі словами
{
	string tempLine = "";
	for (auto line : data) {
		tempLine += line + " ";
	...
	tempLine.pop_back();
	return tempLine;   //повернення слів об'єднаних через пробіл
}
